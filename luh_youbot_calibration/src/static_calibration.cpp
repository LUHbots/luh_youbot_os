...
int main(int argc, char** argv)
{
    ...
    else
    {
        ...
        if (posesFile.is_open())
        {
            ...
            std::cout << "totalPoseNumber" << totalPoseNumber << std::endl;

            //Write values q1, q2, q, q4, q5 in poseFile to poseMatrix
            double posesMatrix [totalPoseNumber] [6];
            int poseNumber = -1;
            posesFile.clear();
            ...
            while(std::getline(posesFile, line))
            {
                ...
                {
                    poseNumber++;
                    posesMatrix [poseNumber] [0] = double (poseNumber);
                    ...
                    posesMatrix [poseNumber] [1] = atof (buffer);

                    // q2
                    std::getline(posesFile, line, ':');
                    std::getline(posesFile, line);
                    strcpy(buffer,line.c_str());
                    posesMatrix [poseNumber] [2] = atof (buffer);

                    // q3
                    std::getline(posesFile, line, ':');
                    std::getline(posesFile, line);
                    strcpy(buffer,line.c_str());
                    posesMatrix [poseNumber] [3] = atof (buffer);

                    // q4
                    std::getline(posesFile, line, ':');
                    std::getline(posesFile, line);
                    strcpy(buffer,line.c_str());
                    posesMatrix [poseNumber] [4] = atof (buffer);

                    // q5
                    std::getline(posesFile, line, ':');
                    std::getline(posesFile, line);
                    strcpy(buffer,line.c_str());
                    posesMatrix [poseNumber] [5] = atof (buffer);
                }
//                else
//                {
//                    std::cout << "posesFile in wrong format" << std::endl;
//                    break;
//                }
            ...
            //display posesmatrix
            for (uint j = 0; j < poseNumber + 1; j++)
            {
                for (uint i = 0; i < 6; i++)
                {
                    std::cout << posesMatrix [j] [i] << ' ';
                }
                std::cout << std::endl;
            }
            //std::cout << "poseNumber " << poseNumber << std::endl;
                    ...
                    {
                        //Joint pos anfahren
                        pose.setQ1( posesMatrix [i] [1]);
                        pose.setQ2( posesMatrix [i] [2]);
                        pose.setQ3( posesMatrix [i] [3]);
                        pose.setQ4( posesMatrix [i] [4]);
                        pose.setQ5( posesMatrix [i] [5]);

                        ///Abfrage auskommentieren wenn .YAML getestet
                    ...
                    std::cout << "Finished last measurement" << std::endl;
        ...
        std::cout << "Finished last measurement2" << std::endl;
    }
    std::cout << "Finished last measurement3" << std::endl;
